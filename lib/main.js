exports.main = function (options, callbacks) {
  ...
  let datafy_all = function (bw) {
    let out = [];
    for (w of bw) {
      let wrep = [for (tab of w.tabs) if (tab.title !== 'Tab Auditor') {id: tab.id, title: tab.title, url: tab.url, window: w.id}]
      if (w == bw.activeWindow) {
        wrep.active = true;
        out.unshift(wrep);
      }
      else {
  ...
  let attach_auditor = (tab) => {
    let worker = tab.attach({ contentScriptFile: ui_urls.js }),
        ...
        redisplay = function () {
          ...
          show_TO = setTimeout(() => { worker.port.emit("populate", datafy(this))}, 200);
        }
    ...
    worker.port.emit('populate', datafy(tabs));
